...
@credits Ryan Sheffer
Restructured and ported to python 3 on June 10, 2016
"""
import tarfile
import os

def progressprint(complete, path):
    """
...
class TarFile(tarfile.TarFile):
    ...
    def add(self, name, arcname=None, recursive=True, exclude=None, filter=None, progress=None):
        ...
        *progress* will be called with a signal integer with a value between 0 and 100,
        ...
        ..  note::
            Progress will be reported on a file-by-file basis. This means that the percentage
            passed will reset to zero on every new file. This is particularly important is you are
        ...
        return tarfile.TarFile.add(self, name, arcname, recursive, exclude, filter=filter)
    ...
    def extractall(self, path=".", members=None, *, numeric_owner=False, progress=None):
    ...
    def extract(self, member, path="", set_attrs=True, *, numeric_owner=False, progress=None):
